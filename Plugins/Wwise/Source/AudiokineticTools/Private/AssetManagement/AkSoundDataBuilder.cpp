...
bool AkSoundDataBuilder::processMediaEntry(MediaToCookMap& mediaToCookMap, const FString& platform, TArray<TSoftObjectPtr<UAkMediaAsset>>& medias, const TSharedPtr<FJsonObject>& mediaData, bool isStreamed, bool isInitBank)
{
	...
	{
		assetPath = ObjectTools::SanitizeObjectPath(FPaths::Combine(localizedPackagePath, language, AkUnrealHelper::MediaFolderName, stringId + TEXT(".") + stringId));
	}
	else
	{
		language.Empty();

		assetPath = ObjectTools::SanitizeObjectPath(FPaths::Combine(basePackagePath, AkUnrealHelper::MediaFolderName, stringId + TEXT(".") + stringId));
	}
